...
public class ActivityMain extends FragmentActivity implements LocationListener, CallbackResponseListener {
    ...
    private LinearLayout ll_FairCashCredit, ll_cancelConfirm, vehicle_balloon, overlayTrip, timer_view, layout_later, ll_pick_address, ll_drop_address, ll_selected_time, ll_selected_cab, ll_getting_nearby_progress;
    private RelativeLayout ll_pn_pl, rl_send_nearest, history, rl_bottom_views, rl_address_pointer, app_action_bar;
    ...
    private TextView tv_app_version, tv_timer, tv_pick_drop, tv_address, tv_pick_address, tv_drop_address, tv_sendme, tv_nearestcab, tv_selectedcab, tv_fare_estimate, tv_mile_estimate, menu_header, tv_refresh, tv_airport_note;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        rl_bottom_views = (RelativeLayout) findViewById(R.id.rl_bottom_views);
        ...
        if (BookingApplication.bShowNearbyPlaces) {
            ...
            rl_bottom_views.setLayoutParams(layoutParams);

            Animation outAnim = AnimationUtils.loadAnimation(ActivityMain.this, R.anim.slide_in_bottom);
        ...
        rl_address_pointer = (RelativeLayout) findViewById(R.id.rl_address_pointer);
        rl_address_pointer.setBackgroundResource(BookingApplication.textView_Background);
        ...
        cb_fav_drop = (CheckBox) findViewById(R.id.fav_drop);
        cb_fav_pick = (CheckBox) findViewById(R.id.fav_pick);
    ...
    public void picknow(View v) {
        ...
        //rl_bottom_views.setVisibility(View.GONE);
    ...
    public void cancelDrop(View v) {
        ...
        if (BookingApplication.getAffiliatesIDFromDB(ActivityMain.this).size() > 0) {
            ...
            if (isQuickBooking || trip_requested.companyID > 0)
                goToStep(5, false);
            else
                goToStep(3, false);
        }
        ...
        if (selecting_pickup)
            try {
                ...
                if (dropMarkerOptions != null) {
                    ...
                    rl_bottom_views.setVisibility(View.GONE);
                    rl_address_pointer.setVisibility(View.GONE);
                    selecting_drop = false;
                    if (dropMarkerOptions != null)
            ...
            try {
                ...
                if (BookingApplication.favorites.contains(new Addresses("", "", new LatLng(trip_requested.DOlat, trip_requested.DOlong))))
                    cb_fav_drop.setChecked(true);
                else
                    cb_fav_drop.setChecked(false);
                ...
                public void onCameraChange(final CameraPosition position) {
                    try {
                        ...
                        if (selecting_pickup || selecting_drop) {
                            ...
                            if (BookingApplication.bShowNearbyPlaces && placesList.size() == 0)
    ...
    /*------------------------------------------------ showHideBottomViews ---------------------------------------------------------------------------------*/
    public void showHideBottomViews(View v) {
        if (!lv_nearby_places.isShown()) {
            RelativeLayout.LayoutParams layoutParams = new RelativeLayout.LayoutParams(RelativeLayout.LayoutParams.MATCH_PARENT, (int) (BookingApplication.screenHeight / 2.5));
            layoutParams.addRule(RelativeLayout.ALIGN_PARENT_BOTTOM);
            layoutParams.addRule(RelativeLayout.ABOVE, R.id.ll_cancelConfirm);
            rl_bottom_views.setLayoutParams(layoutParams);
            ...
            tt = new TimerTask() {
                ...
                public void run() {
                    try {
                        final List<Place> places = google.getNearbyPlaces(BookingApplication.currentLatLong.latitude, BookingApplication.currentLatLong.longitude, 300, new Param[]{new Param("types").value("establishment"), new Param("rankBy").value("distance")});
    ...
    /*------------------------------------------------ showTripConfirmDialog ---------------------------------------------------------------------------------*/
    public void showTripConfirmDialog(boolean isNowTrip) {

        LayoutInflater inflater = (LayoutInflater) getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        View layout = inflater.inflate(R.layout.dialog_confirm_trip, null);

        final BookingApplication.CustomDialog bb = new BookingApplication.CustomDialog(this, layout);

        TextView message_text = (TextView) layout.findViewById(R.id.tv_message_text);

        if (isNowTrip) {
            message_text.setText(getString(R.string.NowBookingMsg));
        } else {
            message_text.setText(getString(R.string.LaterBookingMsg));
        }

        TextView btn_Confirm = (TextView) layout.findViewById(R.id.btnConfirm);
        TextView btn_CANCEL = (TextView) layout.findViewById(R.id.btnCANCEL);
