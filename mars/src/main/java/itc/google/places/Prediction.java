...
import static itc.google.places.GooglePlaces.checkStatus;
...
public class Prediction {
     ...
     */
    public static List<Prediction> parse(GooglePlaces client, String rawJson) throws JSONException {
        JSONObject json = new JSONObject(rawJson);
        checkStatus(json.getString(GooglePlacesInterface.STRING_STATUS), json.optString(GooglePlacesInterface.STRING_ERROR_MESSAGE));

        List<Prediction> predictions = new ArrayList<Prediction>();
        JSONArray jsonPredictions = json.getJSONArray(GooglePlacesInterface.ARRAY_PREDICTIONS);
        for (int i = 0; i < jsonPredictions.length(); i++) {
            JSONObject jsonPrediction = jsonPredictions.getJSONObject(i);
            String placeId = jsonPrediction.getString(GooglePlacesInterface.STRING_PLACE_ID);
            String description = jsonPrediction.getString(GooglePlacesInterface.STRING_DESCRIPTION);

            JSONArray jsonTerms = jsonPrediction.getJSONArray(GooglePlacesInterface.ARRAY_TERMS);
            ...
            JSONArray jsonTypes = jsonPrediction.optJSONArray(GooglePlacesInterface.ARRAY_TYPES);
            ...
            JSONArray substrArray = jsonPrediction.getJSONArray(GooglePlacesInterface.ARRAY_MATCHED_SUBSTRINGS);
